...
class UserController extends Controller
{
    ...
    /**
     * @Route("/{id}/image/", name="image.user")
     * @Method({"GET"})
     */
    public function getUserImageAction($id)
    {
        ...
        $image = $user->getImage() != null ? $user->getImage() : $defaultImage; 
    ...
    private function saveUserData(User $user, $request)
    {
        ...
        $user->setImage($request->get('image', $user->getImage()));
    ...
    private function getUserAsArray($user)
    {
        ...
        return array(
            ...
            'image' => $this->generateUrl('image.user', array('id' => $user->getId()), true)
        );
    }
}
